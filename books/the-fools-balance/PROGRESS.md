# Progress

## Current Volume

Volume 1:
The Fool Who Challenged Heroes


---

# World

[x] Core concept

[x] Kingdom

[x] Hero system

[x] Magic philosophy

[ ] Geography

[ ] History

[ ] Religion


---

# Characters

Locked cores (see `lore/characters/`):

[x] The Fool (Harlequin Akihiko)

[x] Princess Ayame

[x] Renji Arata

[x] Emperor Seiryu

[x] Inquisitor Kuro

[x] Roster scaffold (`lore/characters/roster-index.md`) — empty slots for long-series cast

[x] Wave 1 named (AS01–AS06, HC01, ST01, EX01, SC01–SC05)

[ ] Wave 2: ST02–ST03, EX02–EX04 (first holy conflict)

[ ] Villains (Demon Lords + Generals)

[ ] Remaining supporting cast (Academy / Guilds / Royals / side)


---

# Story

[x] Main conflict

[x] Volume structure

[x] Chapter outlines (Ch 1–10 scene map)

[x] Chapter 1 draft accepted — The Fool Arrives (`chapters/chapter-01.md`)

[x] Chapter 2 draft accepted — The Man Who Lost the Fight Before It Began (`chapters/chapter-02.md`)

[x] Chapter 3 draft — Shape of a Villain (`chapters/chapter-03.md`) — pending accept

[ ] Remaining chapter drafts

[x] PDF export script + test build (`scripts/export_pdf.py` → `export/Fools-Balance-Volume-1.pdf`) — rebuilt after Ch 2 polish


---

# Mysteries

[x] The Fool's identity

[x] Origin of Infringement

[ ] Why he became The Fool

[ ] The First Law

[ ] The Previous Fool


---

# Tracking (after Ch 2 accept)

[x] `tracking/canon.md`

[x] `tracking/continuity.md`

[x] `tracking/foreshadowing.md`

[x] `tracking/unresolved-threads.md`

[x] `tracking/character-arcs.md`

[x] `tracking/chapter-status.md`

[x] Roster statuses (M01/M02 introduced Ch 1–2; HC01 cited)


---

# Current Goal

Draft Chapter 3 (Princess Ayame / royal court).

Volume goal remains: Complete Volume 1.

Target:

70 chapters

Average chapter:

3000-5000 words

Total:

250k-350k words