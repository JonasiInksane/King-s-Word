# Character Roster Index

Scaffold for a long-series cast. **Names stay TBD until deliberately filled.**

Do not write full bios here. Each entry needs only:

| Field | Meaning |
|---|---|
| **Name** | Locked when introduced |
| **Philosophy** | One sentence — control / freedom / truth / order |
| **Ability** | Rei discipline (Shiki / Kotodama / Kage) + Resonance hook, or none |
| **Fool vector** | How they misread The Fool |
| **Status** | `empty` · `named` · `introduced` · `recurring` · `dead` · `offstage` |

**Main cast bios** live in individual files. This index tracks depth for 250–350 chapters.

**Canon anchors** ([`tracking/canon.md`](../../tracking/canon.md)):

- World: Akaritsu · Kingdom: Akatsuki · Fool: Harlequin Akihiko
- Emperor Seiryu · Princess Ayame · Inquisitor Kuro
- Magic: Rei (Shiki / Kotodama / Kage) · Axioms · Resonance

**Wave rule:** Never introduce a full set on-page until that institution matters. Fill slots as arcs need them.

---

# Main Cast (locked cores)

| ID | Name | File | Philosophy | Fool vector | Status |
|---|---|---|---|---|---|
| M01 | Harlequin Akihiko | [harlequin-akihiko.md](harlequin-akihiko.md) | Roles themselves are the enemy | — (camera rarely; Ch 5 paid) | introduced Ch 1; Academy Ch 4; holding Ch 5 |
| M02 | Renji Arata | [sir-renji.md](sir-renji.md) | Order protects; rules restrain humans | Criminal → madman → necessary equal? | introduced Ch 1; Academy bind Ch 4; holding Ch 5 |
| M03 | Emperor Seiryu | [emperor-seiryu.md](emperor-seiryu.md) | Control creates peace | Greatest threat (exposes imperfect system) | introduced Ch 3 (court; Ayame warning) |
| M04 | Princess Ayame | [princess-ayame.md](princess-ayame.md) | Information controls reality | Sees the performance; arrest or follow? | introduced Ch 3 (court + trail); offstage Ch 4–5 |
| M05 | Inquisitor Kuro | [inquisitor-kuro.md](inquisitor-kuro.md) | Chaos = suffering; unpredictability must die; “Agreement without judgment” | Ultimate ideological enemy | introduced Ch 5–6 (POV Ch 6) |
| M06 | Wataru | — | Write what you see; stay ordinary | Reader’s Hero / normal eyes on the Fool | introduced Ch 6; **do not kill or betray** |

---

# 12 Hero Commanders

Elite field leaders of the Imperial Hero structure (above Academy graduates).

| # | Name | Philosophy | Ability | Fool vector | Status |
|---|---|---|---|---|---|
| HC01 | Hoshikawa Ryo | Strength without spectacle is virtue | Shiki — Iron Frame; Resonance seed: Weight (force lands heavier when he decides it must) | Clown wasting talent; draft him or break him into the system | named (cited Ch 1 dispatch; not on-page) |
| HC02 | TBD | | | | empty |
| HC03 | TBD | | | | empty |
| HC04 | TBD | | | | empty |
| HC05 | TBD | | | | empty |
| HC06 | TBD | | | | empty |
| HC07 | TBD | | | | empty |
| HC08 | TBD | | | | empty |
| HC09 | TBD | | | | empty |
| HC10 | TBD | | | | empty |
| HC11 | TBD | | | | empty |
| HC12 | TBD | | | | empty |

---

# 7 Saints

Empire's strongest holy warriors (Holy Dominion). Overview seed: **Seven Saints**.

| # | Name | Philosophy | Ability | Fool vector | Status |
|---|---|---|---|---|---|
| ST01 | Hikari Nozomi | Mercy that never punishes becomes cruelty | Kotodama — Still / Bind; Axiom seed: Sanctum (lies thin inside her light) | Blasphemous jester; convert or cleanse | named |
| ST02 | TBD | | | | empty |
| ST03 | TBD | | | | empty |
| ST04 | TBD | | | | empty |
| ST05 | TBD | | | | empty |
| ST06 | TBD | | | | empty |
| ST07 | TBD | | | | empty |

---

# 12 Executioners (Holy Inquisition)

Elite arms of the Inquisition. Overview seed: **Twelve Executioners**.  
(Public may say "Inquisitors"; roster title stays Executioners unless canon revises.)

Kuro sits above this table as leader — not one of the twelve.

| # | Name | Philosophy | Ability | Fool vector | Status |
|---|---|---|---|---|---|
| EX01 | Shigure | Silence is kindness to the guilty | Kage — Memory Cut (short blanks at the moment of strike) | Kuro's test target; kill him cleanly, without hate | introduced Ch 6 |
| EX02 | TBD | | | | empty |
| EX03 | TBD | | | | empty |
| EX04 | TBD | | | | empty |
| EX05 | TBD | | | | empty |
| EX06 | TBD | | | | empty |
| EX07 | TBD | | | | empty |
| EX08 | TBD | | | | empty |
| EX09 | TBD | | | | empty |
| EX10 | TBD | | | | empty |
| EX11 | TBD | | | | empty |
| EX12 | TBD | | | | empty |

---

# Demon Forces

Overview seed: **Four Demon Lords**. Expanded for series depth:

- 4 Lords (apex)
- 8 Generals (under Lords — user's long-series target)

## 4 Demon Lords

| # | Name | Philosophy | Ability | Fool vector | Status |
|---|---|---|---|---|---|
| DL01 | TBD | | | | empty |
| DL02 | TBD | | | | empty |
| DL03 | TBD | | | | empty |
| DL04 | TBD | | | | empty |

## 8 Demon Generals

| # | Name | Serves Lord | Philosophy | Ability | Fool vector | Status |
|---|---|---|---|---|---|---|
| DG01 | TBD | TBD | | | | empty |
| DG02 | TBD | TBD | | | | empty |
| DG03 | TBD | TBD | | | | empty |
| DG04 | TBD | TBD | | | | empty |
| DG05 | TBD | TBD | | | | empty |
| DG06 | TBD | TBD | | | | empty |
| DG07 | TBD | TBD | | | | empty |
| DG08 | TBD | TBD | | | | empty |

---

# Hero Academy — 20+ Students

Most are arc-local. Mark `recurring` only if they keep a Fool relationship across volumes.

Target: **20 named minimum** (expand rows as needed).

| # | Name | Year/Track | Philosophy | Ability | Fool vector | Status |
|---|---|---|---|---|---|---|
| AS01 | Sora Hayashi | Y2 · Field Hero | Heroes exist to be understood by the people they save | Shiki — Wind Step | Secret teacher testing the Academy | named |
| AS02 | Yui Takamura | Y3 · Rank Track | Rank is truth written in public | Kotodama — Measure (binds contests to stated rules) | Cheat mocking the ladder; must be exposed | introduced Ch 4 |
| AS03 | Kenji Morita | Y1 · Commoner | Power means nothing if it never reaches the street | Shiki — Stubborn Frame | Fraud… until he saves someone Kenji loves | introduced Ch 4 (POV) |
| AS04 | Rina Kagetsu | Y3 · Noble Track | Bloodline is a debt, not a crown | Kage — Soft Veil (crowd attention slides off her) | Rival-house weapon wearing a smile | named |
| AS05 | Daichi Orochi | Y2 · Combat | Hierarchy is mercy; chaos is selfishness | Shiki — Crush Grip; Resonance seed: Burden | Weak for laughing; prove the order with fists | introduced Ch 4 |
| AS06 | Mei Suzume | Y2 · Support | Healing that ignores the wound's cause is theater | Kotodama — Mend / Quiet | Lonely man pretending; try to help him | introduced Ch 4 |
| AS07 | TBD | | | | | empty |
| AS08 | TBD | | | | | empty |
| AS09 | TBD | | | | | empty |
| AS10 | TBD | | | | | empty |
| AS11 | TBD | | | | | empty |
| AS12 | TBD | | | | | empty |
| AS13 | TBD | | | | | empty |
| AS14 | TBD | | | | | empty |
| AS15 | TBD | | | | | empty |
| AS16 | TBD | | | | | empty |
| AS17 | TBD | | | | | empty |
| AS18 | TBD | | | | | empty |
| AS19 | TBD | | | | | empty |
| AS20 | TBD | | | | | empty |
| AS21 | TBD | | | | | empty |
| AS22 | TBD | | | | | empty |
| AS23 | TBD | | | | | empty |
| AS24 | TBD | | | | | empty |

---

# 15 Guild Leaders

Economy / information. Overview seed: **Merchant Guild Masters**.

| # | Name | Guild | Philosophy | Ability | Fool vector | Status |
|---|---|---|---|---|---|---|
| GL01 | TBD | | | | | empty |
| GL02 | TBD | | | | | empty |
| GL03 | TBD | | | | | empty |
| GL04 | TBD | | | | | empty |
| GL05 | TBD | | | | | empty |
| GL06 | TBD | | | | | empty |
| GL07 | TBD | | | | | empty |
| GL08 | TBD | | | | | empty |
| GL09 | TBD | | | | | empty |
| GL10 | TBD | | | | | empty |
| GL11 | TBD | | | | | empty |
| GL12 | TBD | | | | | empty |
| GL13 | TBD | | | | | empty |
| GL14 | TBD | | | | | empty |
| GL15 | TBD | | | | | empty |

---

# 10 Royal Families

Akatsuki Imperial line is primary. Others are satellites / rivals / marriage politics.  
Overview seeds: Crown + **Hidden Court**.

| # | House / Family | Realm | Head (TBD) | Philosophy | Fool vector | Status |
|---|---|---|---|---|---|---|
| RF01 | Akatsuki Imperial | Akatsuki | Seiryu / Ayame | Control / information | See main cast | named |
| RF02 | TBD | | | | | empty |
| RF03 | TBD | | | | | empty |
| RF04 | TBD | | | | | empty |
| RF05 | TBD | | | | | empty |
| RF06 | TBD | | | | | empty |
| RF07 | TBD | | | | | empty |
| RF08 | TBD | | | | | empty |
| RF09 | TBD | | | | | empty |
| RF10 | TBD (Hidden Court link?) | | | | | empty |

---

# 30+ Recurring Side Characters

Witnesses, rivals, informants, victims, comic relief with teeth.  
Only promote to `recurring` if they survive more than one arc with a Fool vector.

| # | Name | Role | Philosophy | Ability | Fool vector | Status |
|---|---|---|---|---|---|---|
| SC01 | Old Tetsu | Academy groundskeeper | Walls remember better than students | None (knows locks, roofs, schedules) | Pest who somehow knows where the keys live | introduced Ch 4 |
| SC02 | Isamu | Street vendor (bells & gossip) | People pay for stories, not truth | None | Traveling actor; good for business either way | named |
| SC03 | Captain Mori | Capital city watch | Laws are doors with hinges — oil them or they scream | Weak Shiki | Arrest for paperwork; the joke is the paperwork | introduced Ch 3 |
| SC04 | Archivist Nao | Academy / royal librarian | Unfiled knowledge is a crime against tomorrow | Minor Kotodama — Index (names stick when spoken to ledgers) | Uncatalogued anomaly walking the stacks | named |
| SC05 | Priestess Emi | Outer Chapel (Holy Dominion fringe) | Fear of god is cheaper than love of god | Minor Kotodama — Prayer Bind | Bad omen; half believes her own sermon | named |
| SC06 | TBD | | | | | empty |
| SC07 | TBD | | | | | empty |
| SC08 | TBD | | | | | empty |
| SC09 | TBD | | | | | empty |
| SC10 | TBD | | | | | empty |
| SC11 | TBD | | | | | empty |
| SC12 | TBD | | | | | empty |
| SC13 | TBD | | | | | empty |
| SC14 | TBD | | | | | empty |
| SC15 | TBD | | | | | empty |
| SC16 | TBD | | | | | empty |
| SC17 | TBD | | | | | empty |
| SC18 | TBD | | | | | empty |
| SC19 | TBD | | | | | empty |
| SC20 | TBD | | | | | empty |
| SC21 | TBD | | | | | empty |
| SC22 | TBD | | | | | empty |
| SC23 | TBD | | | | | empty |
| SC24 | TBD | | | | | empty |
| SC25 | TBD | | | | | empty |
| SC26 | TBD | | | | | empty |
| SC27 | TBD | | | | | empty |
| SC28 | TBD | | | | | empty |
| SC29 | TBD | | | | | empty |
| SC30 | TBD | | | | | empty |
| SC31 | TBD | | | | | empty |
| SC32 | TBD | | | | | empty |
| SC33 | TBD | | | | | empty |
| SC34 | TBD | | | | | empty |
| SC35 | TBD | | | | | empty |

---

# Other seeds (index later)

Not full tables yet — spin out when an arc needs them:

| Group | Notes |
|---|---|
| Hidden Court | May overlap RF10 / off-book power |
| Ancient Watchers | Pre-Empire memory; late-series |
| Royal Academy faculty | Separate from students if needed |

---

# Fill order (recommended)

1. ~~Vol 1 faces: AS01–AS06, HC01, ST01, EX01, SC01–SC05~~ **done (Wave 1)**  
2. First holy conflict: ST02–ST03, EX02–EX04  
3. First demon pressure: DL01, DG01–DG02  
4. Economy arc: GL01–GL05  
5. Expand only when a slot must appear on-page  

---

# Counts (targets)

| Group | Target | Filled (named+) |
|---|---|---|
| Main cast | 5 | 5 (M01–M05 introduced through Ch 6) |
| Hero Commanders | 12 | 1 (HC01 cited Ch 1) |
| Saints | 7 | 1 (ST01) |
| Executioners | 12 | 1 (EX01) |
| Demon Lords | 4 | 0 |
| Demon Generals | 8 | 0 |
| Academy Students | 20+ | 6 (AS02/03/05/06 on-page Ch 4; AS01/04 named) |
| Guild Leaders | 15 | 0 |
| Royal Families | 10 | 1 (Akatsuki) |
| Side characters | 30+ | 5 (SC01 Tetsu Ch 4; SC03 Mori Ch 3; SC02/SC04–SC05 named) |
