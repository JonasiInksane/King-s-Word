#!/usr/bin/env python3
"""
Export The King's Word as a clean novel PDF.
...
COVER_CANDIDATES = [
    ...
    ASSETS_DIR / "covers" / "cover.png",
    ASSETS_DIR / "kings-word-book-cover.png",
    ROOT / "Generated_image.png",
]

BOOK = {
    "title_jp": "王の言霊",
    "title_en": "The King's Word",
    "book_label": "Book One",
    "author": "",
}
...
class NovelPDF(FPDF):
    ...
    def add_full_page_cover(self, image: Path) -> None:
        """Full-bleed cover: whole image on the page (no crop). Slight A4 stretch OK."""
        prev_l, prev_t, prev_r = self.l_margin, self.t_margin, self.r_margin
        self.set_margins(0, 0, 0)
        self.set_auto_page_break(auto=False)
        self.add_page()
        self._no_footer.add(self.page_no())
        # Exact page box — previous "cover-crop" mode cut off the bottom title art.
        self.image(str(image), x=0, y=0, w=self.w, h=self.h)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Export The King's Word as a novel PDF")
    ...
    out_path = args.out / "Kings-Word-Book-1.pdf"
